...
IZIN=$( curl https://raw.githubusercontent.com/leadvpn/ip/main/access | grep $MYIP )
if [ $MYIP = $IZIN ]; then
clear
echo -e "${green}Tahniah! Anda Dibenarkan menggunakan LEADVPN...${NC}"
sleep 5
else
clear
echo ""
...
echo ""  | tee -a log-install.txt
echo ""
echo "   - DEV TELEGRAM            : @rudy3126"  | tee -a log-install.txt
echo "   - Whatsapp                : +60138852423"  | tee -a log-install.txt
echo ""
echo "------------------Script Created By LEADVPN---------------------" | tee -a log-install.txt
